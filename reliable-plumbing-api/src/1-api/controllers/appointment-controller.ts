...
export class AppointmentController {
  ...
  addAppointment(@BodyParam('appointment') appointmentModel, @UploadedFiles('images', { options: fileUploadOptions }) images) {
    ...
    return new Promise<any>((resolve, reject) => {
      this.appointmentManager
        .addAppointment(appointment, images)
  ...
  getAssigneesAppointments(@Body() filters) {
    return new Promise<Appointment[]>((resolve, reject) => {
      this.appointmentManager
        .getAssigneesAppointments(filters.assigneeIds, filters.from, filters.to)
        .then(appointments => {
          // to light models here
          let models = [];
          for (let appointment of appointments) models.push(appointment.toLightModel());
  ...
  getTechniciansWithStatusInTime(@QueryParam('appointmentId') appointmentId: string) {
    return new Promise<any>((resolve, reject) => {
      this.appointmentManager
        .getTechniciansWithStatusInTime(appointmentId)
        .then(results => {
          let models = [];
  ...
  updateAppointmentStatusAndAssignees(@Body() appointment) {
    return new Promise<any>((resolve, reject) => {
      this.appointmentManager
        .updateAppointmentStatusAndAssignees(appointment)
